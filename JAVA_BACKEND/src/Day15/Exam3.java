...
public class Exam3 {
    public static void main(String[] args) {
        /*  프로그램: 실행 가능한 코드들의 집합
            ...
            * 활용처 : 1) 톰캣(웹서버/SPRING): 다수의 유저(클라이언트)가 1개의 서버에 요청시 병렬처리를 제공
        ...
        */
        ...
        //[2] 멀티스레드 방법1: 익명 구현체 , new 인터페이스타입(){오버라이딩};
        Runnable runnable = new Runnable() { 
            // 새로운(작업) 스레드가 처리할 메소드    / run( ) 오버라이딩(재정의)
            @Override
            public void run() { // 추상메소드 
                for(int i =1; i<=5; i++){
                    System.out.println(">첫번째 새로운 스레드: " + i);
                    try{ Thread.sleep( 1000 ); }catch( Exception e ){ }
                }
            }
        };  // 익명구현체 구현 끝
