...
public class Exam1 {
    public static void main(String[] args) {
        ...
        /*
        배열 : 여개 (동일한타입)자료들을 저장하는 타입, 정적길이
        ArrayList 클래스 :
            1. 컬랙션(수집) 프레임(틀)워크(일), 자료 수집관련 클래스 / 기능 제공
            2. 목적 : 가변길이 , 배열 관련된 기능(메소드) 제공한다. <편리하다>
            3. 사용법
                ArrayList<제네릭타입> 변수명 = new ArrayList<>();
                    - 제네릭타입 : 리스트내 저장할 요소들의 타입, 즉> <String> 리스트에 여러개 String 저장
                    - 주의할 점 : <제네릭타입> 은 기본타입이 불가능하다. int -> Integer 레퍼타입 (기본타입을 클래스타입)
            4. 주요키워드
                1.  add(자료) : 리스트 요소 추가
    ...
    //  [1] 리스트 생성
        ...
        // [5] 리스트 요소 수정
        변수명1.set(1,"서장훈");   // 요소 수정
        // [6] 리스트 요소 총계수
        System.out.println(변수명1);
        변수명1.size();                                           // 리스트 요소 총계수
        // [7] 리스트 특정 요소 출력
        ...
        System.out.println(변수명1);
        //[9] 리스트 요소 찾기
        System.out.println(변수명1.indexOf("강호동"));      //  , 찾는값의 인덱스 반환 , 없으면 -1
        System.out.println(변수명1.contains("강호동"));     // 찾는값이 있으면 True , 없으면 false
        // [10] 반복문과 리스트 관계
        // 1. 일반 for문
        for(int i = 0; i<= 변수명1.size()-1;i++){
        }
        // 2. 향상된 for문      : 콜론기준 오른쪽 리스트내 요소 하나씩 왼쪽에 대입.
        for(String str : 변수명1){
        }   // 당분간 해당 방식 활용해보면 좋을듯 함. 
        ...
        // [12] 리스트 비어있는지 요소 여부 확인.
        System.out.println(변수명1.isEmpty());  //이 기능 생각보다 많이 활용됨.         리스트 내 요소가 하나도 없으면 True, 아니면 false.
        System.out.println(변수명1);
