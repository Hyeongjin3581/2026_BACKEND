...
public class practice12 {
    public static void main(String[] args) {
        ...
        s1.name = "정형진";
        s1.studentId = 10;
        System.out.println(s1.name);    System.out.println(s1.studentId);
        ...
        //[3]
        // * 생성자내부에는 super() 생략시 자동으로 적용됨. 즉, super() 키워드는 부모 생성자 호출 키워드
        Computer computer = new Computer();

        //[4] 3. main 함수에서 Triangle 객체를 생성하여 Figure 타입의 변수에 저장하는 코드를 작성하고, 이것이 가능한 이유를 주석으로 설명하세요.
        Figure triangle = new Triangle();   // 부모 클래스인 Figure에 지역변수가 triangle인 new Triangle 객체가 생성가능한 이유는, Triangle이 자식클래스이며, 자식 타입의 객체가 부모 타입의 참조 변수에 대입될 때 업캐스팅이 일어나 다형성을 구현할 수 있기때문
        // [5] 3.main 함수에서 Shape shape = new Circle(); 코드를 작성한 뒤, shape.draw()를 호출했을 때 어떤 결과가 나오는지 확인하고 그 이유를 주석으로 설명하세요.*/
        Shape shape = new Circle();
        shape.draw();   // 상속받은 Circle클래스에서 draw 메서드를 오버라이드하여 재정의 하여서.
